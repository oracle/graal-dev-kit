...
public class GdkFeatureContext extends FeatureContext {
    ...
    private final Map<GdkCloud, Collection<Feature>> addedFeatures = new HashMap<>();
    ...
    public GdkFeatureContext(Options options,
        ...
        generateExampleCode = getOptions().get(GdkGeneratorContext.EXAMPLE_CODE, Boolean.class).orElse(true);
    }
    ...
    public void processSelectedFeatures() {
        ...
        for (GdkCloud cloud : selectedClouds) {
    ...
    private void processSelectedFeatures(GdkCloud cloud) {
        this.cloud = cloud;
        ...
        features.addAll(selectedAndDefaultFeatures.stream()
                .filter(f -> {
                    ...
                    if (f instanceof GdkFeature) {
                        return cloud == ((GdkFeature) f).getCloud();
                    }
     ...
     * from {@link AbstractGdkFeature#processSelectedFeatures(GdkFeatureContext)}, and
     * this happens in <code>GdkContextFactory</code> before the generator
     ...
     * here. Later when GdkGeneratorContext is being constructed, the cloud
     ...
     */
    @NonNull
    public Set<GdkCloud> getAddedFeatureClouds(Feature feature) {

        Set<GdkCloud> clouds = new HashSet<>();

        for (Map.Entry<GdkCloud, Collection<Feature>> e : addedFeatures.entrySet()) {
            if (e.getKey() == NONE) {
                continue;
            }
     ...
     * the GDK "create" and "service" features that were auto-selected based on
