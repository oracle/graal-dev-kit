...
public class GraalvmQuickBuild implements Feature {
    ...
    public void apply(GeneratorContext ctx) {
        if (ctx.getBuildTool().isGradle()) {
            ...
            if (!((GcnGeneratorContext) ctx).getCloud().getModuleName().isBlank()) {
                template = template + "-" + ((GcnGeneratorContext) ctx).getCloud().getModuleName();
            }
            ((GcnGeneratorContext) ctx).addPostProcessor(template, GRADLE_POST_PROCESSOR);
        } else {
            String template = "mavenPom";
            if (!((GcnGeneratorContext) ctx).getCloud().getModuleName().isBlank()) {
                template = template + "-" + ((GcnGeneratorContext) ctx).getCloud().getModuleName();
            }
            ((GcnGeneratorContext) ctx).addPostProcessor(template, MAVEN_POST_PROCESSOR);
        }
    }
