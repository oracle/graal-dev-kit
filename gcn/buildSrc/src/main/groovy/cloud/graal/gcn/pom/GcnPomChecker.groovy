...
@CompileStatic
@CacheableTask
abstract class GcnPomChecker extends DefaultTask {
    ...
    void verifyBom() {
        ...
        while (!queue.isEmpty()) {
            ...
            queue.each { item ->
                ...
                if (seen.add(key)) {
                    workQueue.submit(CheckGcnPomAction) { params ->
            ...
            reports.each {
                ...
                validation.invalidDependencies.each {
                    errorCollector.error("$bomPrefix declares a non-resolvable dependency: $it".toString())
                }
            }
        }
